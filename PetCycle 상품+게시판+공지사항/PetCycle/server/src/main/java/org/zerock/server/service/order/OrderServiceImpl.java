...
public class OrderServiceImpl implements OrderService {
    ...
    public Long registerTransaction(TransactionRegisterRequestDto dto, UserDetails userDetails) {
        ...
        // 이미 거래된 상품 방지
        if (transactionHistoryRepository.findByProduct_ProductId(product.getProductId()).isPresent()) {
            throw new IllegalArgumentException("이미 거래된 상품");
        }

        String status = "판매";
        if (product.getStatus() != null) status = product.getStatus();
