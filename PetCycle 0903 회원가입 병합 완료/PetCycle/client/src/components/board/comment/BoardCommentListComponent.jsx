...
/** (옵션) 응답 평탄화: 서버가 user:{id,nickname}처럼 내려올 수도 있을 때 대비 */
...
const BoardCommentListComponent = ({
  ...
  // 쿠키에서 로그인 사용자 정보 세팅 (userId + nickname)
  useEffect(() => {
  ...
  const fetchComments = useCallback(async () => {
    ...
    try {
      ...
      // 평탄화해두면 이후 로직이 단순해짐 (userId / nickname 통일)
  ...
  const renderOne = (c, isChild = false) => {
    // 1순위: userId 매칭, 2순위: nickname 매칭 (서버에서 userId 안 주는 현재 상황 대응)
    const byId =
      myUserId != null &&
      c?.userId != null &&
