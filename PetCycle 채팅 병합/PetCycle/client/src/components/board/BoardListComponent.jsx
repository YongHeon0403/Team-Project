...
const BoardListComponent = () => {
  ...
  // ✅ [FIX] 페이지 이동 시에도 현재 필터(type/keyword)를 항상 유지하도록 래퍼 콜백 추가
  // 기존 moveToList는 유지하고, PageComponent에만 이 래퍼를 넘겨 필터가 빠지지 않게 함
  const movePageWithFilter = (next = {}) => {
    // next는 보통 { page, size }만 전달됨
    moveToList({
      page: next.page ?? page,
      size: next.size ?? size,
      type,      // 🔒 현재 선택된 필터 유지
      keyword,   // 🔒 현재 선택된 필터 유지
    });
  };
  ...
  return (
    <div className="border border-blue-600 mt-4 mx-2 rounded">
      ...
      {/* ✅ [FIX] PageComponent에 movePage 대신 movePageWithFilter 전달 */}
      <PageComponent serverData={serverData} movePage={movePageWithFilter} />
    </div>
  );
};
