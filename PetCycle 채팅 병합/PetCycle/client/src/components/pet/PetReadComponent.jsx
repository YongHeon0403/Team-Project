...
export default function PetReadComponent() {
    ...
    const load = async () => {
      ...
      try {
        ...
        const normalized = {
          ...
          name: data.name ?? "",
          petTypeId: Number(data.petTypeId ?? data.pet_type_id ?? data.typeId) || 3,
  ...
  if (!pet)
    return <div className="py-12 text-center text-gray-400">데이터가 없습니다.</div>;
    ...
    <div className="max-w-3xl space-y-6">
      <div className="flex gap-6">
        ...
        <div className="flex-1 space-y-2">
          ...
          {pet.description && (
            <p className="text-gray-800 whitespace-pre-wrap">{pet.description}</p>
          )}
      ...
      <div className="flex gap-2">
        <button className="px-4 py-2 border rounded" onClick={() => nav("/pet/list")}>
          목록
        </button>
        <button
        ...
        <button className="px-4 py-2 border rounded text-red-600" onClick={onRemove}>
          삭제
        </button>
      </div>
    </div>
