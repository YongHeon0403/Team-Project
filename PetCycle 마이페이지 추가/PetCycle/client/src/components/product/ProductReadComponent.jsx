...
import { getCookie } from "../../util/CookieUtil"; // [추가] 쿠키에서 사용자 토큰/정보를 읽기 위함

// 팀원이 chatApi.jsx를 만들면 이 스텁을 교체
async function createOrGetRoom(productId) {
  console.warn("[stub] createOrGetRoom", { productId });
  return 0;
}

/* ===================== [추가] JWT에서 사용자/권한 보완적으로 꺼내는 유틸 ===================== */
...
const DEFAULT_IMG = "/no_image.png"; // [추가] 이미지 깨질 때 사용할 로컬 플레이스홀더
...
export default function ProductReadComponent() {
  ...
  // [유지] 상세 응답에서 판매자 ID 파싱(서버 구조 대응)
  const sellerId =
    data?.sellerId ?? data?.seller?.userId ?? data?.seller?.id ?? null;

  // [변경] 단순 isOwner에서 → ADMIN도 관리 허용하도록 확장
  const isOwner =
  ...
  const canManage = isOwner || isAdmin; // [추가] 최종 분기 기준: 판매자 또는 관리자

  const onDelete = async () => {
    // [변경] 판매자 또는 관리자만 삭제 가능하도록 가드 강화
      ...
      <div className="flex gap-4">
        <div className="w-1/2">
          <div className="aspect-square bg-gray-50 rounded overflow-hidden">
            <img
              ...
              alt=""
              // [추가] 이미지 로드 실패 시 로컬 플레이스홀더로 1회 교체
              onError={(e) => {
                if (
                  e.currentTarget.src !==
        ...
        <div className="flex-1 space-y-2">
          ...
          <div className="pt-2 flex gap-2">
            {/* [변경] 버튼 분기 기준을 canManage로 통일(판매자 또는 관리자 = 수정/삭제) */}
            {canManage ? (
              <>
