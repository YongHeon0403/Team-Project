...
public class PetProfileServiceImpl implements PetProfileService {
    ...
    public PetProfileDetailResponseDto getPetDetail(Long petId, UserDetails userDetails) {
        ...
        // [유지] 본인만 조회 가능 정책
        if (userDetails == null) throw new SecurityException("권한 없음");
        UserInfo viewer = userInfoRepository.findByEmail(userDetails.getUsername())
                .orElseThrow(() -> new IllegalArgumentException("사용자 없음"));
        if (!pet.getUser().getUserId().equals(viewer.getUserId())) {
            throw new SecurityException("본인 펫만 조회 가능");
        }
