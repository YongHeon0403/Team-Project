...
public class ChatController {
    ...
    //@SendTo 제거: @MessageMapping은 @SendTo를 명시하지 않아도 SimpMessagingTemplate을 사용하여 메시지를 직접 원하는 토픽으로 전송할 수 있습니다
    // 클라이언트가 /app/chat.sendMessage로 메시지를 보낼 때 처리(채팅 메시지를 받아서 모든 구독자에게 브로드캐스트)

    @MessageMapping("/chat.sendMessage")
    public void sendMessage(@Payload ChatMessageDTO chatMessageDTO) { //@Payload 는 들어오는 메시지의 본문을 메서드 매개변수에 바인딩하는 데 사용되는 Spring 의 어노테이션입니다.
